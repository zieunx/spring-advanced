...
### 프록시기술과 한계 - 의존관계 주입
**<JDK 동적 프록시로 생성하면 발생하는 문제>**  
JDK 동적 프록시로 프록시객체를 생성하여 주입하면 인터페이스 기반으로 생성하기 때문에 타겟 구현체 클래스를 알 방법이 없다.  
때문에 원하는 방식으로 동작하지 않는다.  
물론 인터페이스 기반으로 잘 설계된 시스템에서는 이러한 문제가 발생할 일은 없다.  
하지만 경우에 따라 구체클래스로 주입해야 하는 경우가 생길 수 있다.  
이럴 때는 CGLIB 방식으로 생성하면 해결이 된다.